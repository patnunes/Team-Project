...
def user_has_liked_tweet(username, tweetID):
    ...
    isLiked = Likes.objects.filter(
        tweet=tweetID, user=current_user_id).exists()
    if isLiked is False:
        return 0, 0
    else:
        return 0, 1
...
def like_a_tweet(username, tweetID):
    ...
    if isLiked is False:
        ...
        like_entry = Likes(
            tweet_id=tweetID, user_id=current_user_id, timestamp=datetime.now())
        like_entry.save()
        ...
        Likes.objects.filter(
            tweet_id=tweetID, user_id=current_user_id).delete()
