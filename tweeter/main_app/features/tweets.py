...
def save_user_tweets(tweetData):
    user = User.objects.values_list('id').filter(
        username=tweetData['username'])
    tweet = Tweet(user_id=user[0][0], content=tweetData['tweet'],
                  timestamp=datetime.now(), like_counter=0)
    tweet.save()
...
def retrieve_user_tweets(username):
    ...
    tweet_data = Tweet.objects.filter(user_id__in=following_ids).filter(is_comment=0).order_by(
        '-timestamp').values('id', 'user__username', 'content', 'timestamp', 'like_counter', 'parent_tweet')[:5]
...
def fetch_older_tweets(tweetId, username):
    ...
    following_ids = get_following_ids(username)
    more_tweets = Tweet.objects.filter(user_id__in=following_ids).filter(is_comment=0).order_by(
        '-timestamp').values('id', 'user__username', 'content', 'timestamp', 'like_counter', 'parent_tweet')[:5]
...
def CU_page(username):
    ...
    tweet_data = Tweet.objects.filter(user_id=current_user_id).order_by(
        '-timestamp').values('id', 'user__username', 'content', 'timestamp', 'like_counter')
...
def populate_dashboard(username):
    ...
    tweet_data = Tweet.objects.filter(user_id__in=following_ids).order_by(
        '-timestamp').values('id', 'user__username', 'content', 'timestamp', 'like_counter')
